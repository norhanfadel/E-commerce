...
//            session.getTransaction().commit();
    ...
    public boolean isProductRemovedFromCart(int productId, int userId) {
        boolean isRemoved = true;
        Set<Orders> cart = getCart(userId);
        Set<OrderDetails> items;
        Set<OrderDetails> cartProducts = new HashSet<>();
        for (Orders order : cart) {
            if (order.getOrderNum() == 0) {
    ...
    public boolean updateQuantityByProductId(int product_id, int user_id, int quantity) { //
        ...
        if (!ValidateUser.isExist(user)) {
            ...
            if (!isProductRemovedFromCart(product_id, user_id)) {
                int prevQuantity = 0;
    ...
    public int HowMuch(int productId,int userId){
        int productQuantity =  0;
        if (isProductRemovedFromCart(productId,userId)){
            return 0;
        }else {
            Set<Orders> cart = getCart(userId);
            Set<OrderDetails> itemsI;
            ...
            for (OrderDetails item :  cartProducts){
                if (item.getProducts().getProductId()==productId){
                    productQuantity = item.getQuanity();
                    break;
                }
            }
        }
